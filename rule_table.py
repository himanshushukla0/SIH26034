#!/usr/bin/env python3
"""
 LMPC COMPLIANCE ENGINE (SIH26034) — RULE & PENALTY TABLE

 THIS IS THE ONLY PLACE A RULE CITATION OR A PENALTY IS DEFINED.
 Desktop cockpit, terminal cockpit and FastAPI backend all import from here.
 Do not inline a clause reference or a rupee figure anywhere else.

 THE PENALTY REGIME CHANGED ON 01 MAY 2026 (Jan Vishwas Amendment Act, 2026).
 A first contravention of ss.25/29/36(1) is now an IMPROVEMENT NOTICE under s.15(6),
 not a fine. Every "Fine up to Rs 25,000 for first offence" string is repealed law.
 Source: https://www.indiacode.nic.in/bitstream/123456789/2102/1/2009l.pdf (as on 07-05-2026)
"""
...
IMPROVEMENT_NOTICE = "IMPROVEMENT_NOTICE"   # s.15(6) — warning + specified measures
CIVIL_PENALTY = "CIVIL_PENALTY"             # "liable to a penalty" — not a criminal fine
CRIMINAL_FINE = "CRIMINAL_FINE"             # "punished with fine", may carry imprisonment
...
# s.36(1): pre-packaged commodity lacking the declarations required by s.18 + Rule 6
...
# s.36(2): net quantity less than declared. Still criminal after Jan Vishwas.
...
# s.29: quoting a quantity in a non-standard unit ("gms", "ltr")
...
class Rule:
    ...
    remedy: str = ""           # the "measures to secure compliance" for s.15(6)
    amendment_note: str = ""
...
def notice_kind_for(rule_ids: List[str], offence_number: int = 1) -> str:
    """
    Which instrument the officer should be preparing. If every contravention is a first
    offence attracting an improvement notice, the correct instrument is s.15(6) — not a
    show-cause notice. Most first-time label defects now fall here.
    """
...
# First Schedule (Rule 2(e)) — maximum permissible error on net quantity.
# VERIFY against the bare Schedule before the demo. Two readings circulate: this
# alternating percentage/absolute form (OIML R87-derived, what the Schedule prints), and
# a flattened all-percentage form in some secondary sources. They agree at every band
# boundary, so they differ only inside the absolute bands.
# Each entry: (upper bound of declared quantity in g/ml, is_percentage, value)
MPE_TABLE: Tuple[Tuple[float, bool, float], ...] = (
    (50,           True,  9.0),     # up to 50      -> 9%
    (100,          False, 4.5),     # 50 to 100     -> 4.5 g/ml
    (200,          True,  4.5),     # 100 to 200    -> 4.5%
    (300,          False, 9.0),     # 200 to 300    -> 9 g/ml
    (500,          True,  3.0),     # 300 to 500    -> 3%
    (1000,         False, 15.0),    # 500 to 1000   -> 15 g/ml
    (10000,        True,  1.5),     # 1000 to 10000 -> 1.5%
    (15000,        False, 150.0),   # 10000 to 15000-> 150 g/ml
    (float("inf"), True,  1.0),     # above 15000   -> 1%
)
...
def max_permissible_error(declared_qty: float) -> Tuple[float, str]:
    """Return (allowance in g/ml, description) for a declared net quantity in g or ml."""
    if declared_qty <= 0:
